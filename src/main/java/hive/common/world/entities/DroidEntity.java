...
public class DroidEntity extends Monster {
    ...
    public boolean isWithinMeleeAttackRange(LivingEntity target) {
        double range = getAttributeValue(ForgeMod.ENTITY_REACH.get());
    ...
    public EntityDimensions getDimensions(Pose pose) {
        return pose == Pose.SWIMMING ? EntityDimensions.scalable(0.6f, 0.6f) : super.getDimensions(pose);
    }
    ...
    protected void customServerAiStep() {
        ...
        if (ServerConfigs.INSTANCE.pathDebug.get() && level().getGameTime() % 20 == 0) {
            ...
            if (path != null) {
                ...
                Hive.CHANNEL.send(PacketDistributor.TRACKING_ENTITY.with(() -> this), new DebugPathEffectPacket(all, modes));
            }
        }
    }
