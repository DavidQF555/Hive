...
public final class ClientHelper {
    ...
    public static void renderPath(List<BlockPos> path, byte[] modes) {
        ...
        for (int i = 0; i < path.size(); i++) {
            ...
            world.addParticle(particleFor(modes[i]), pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5, 0, 0, 0);
        }
    ...
    private static ParticleOptions particleFor(byte modeOrdinal) {
        return switch (MovementMode.values()[modeOrdinal]) {
            case WALK -> ParticleTypes.FLAME;
            case JUMP -> ParticleTypes.HAPPY_VILLAGER;
            case SWIM -> ParticleTypes.ANGRY_VILLAGER;
        };
    }
