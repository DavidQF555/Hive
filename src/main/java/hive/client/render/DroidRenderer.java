...
public class DroidRenderer extends HumanoidMobRenderer<DroidEntity, HumanoidModel<DroidEntity>> {
    ...
    @Override
    protected void setupRotations(DroidEntity entity, PoseStack pose, float bob, float yRot, float bodyRot, float scale) {
        super.setupRotations(entity, pose, bob, yRot, bodyRot, scale);
        float swimAmount = entity.getSwimAmount(bodyRot);
        if (swimAmount > 0) {
